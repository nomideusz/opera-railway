...
The service streams a GPU-less desktop Opera over WebSockets with the linuxserver.io Selkies stack. Access is gated by HTTP basic auth (`CUSTOM_USER` / generated `PASSWORD`), and the browser profile persists on a volume at `/config`. The `--disable-dev-shm-usage` flag is pre-set in `OPERA_CLI` so Opera runs happily within Railway's container shared memory; append a URL there to open it at launch.

## Common Use Cases

- Use Opera's built-in features from a device where you cannot install it
...
## Dependencies for Opera Hosting

- None — single service, no database

### Deployment Dependencies

- [linuxserver.io Opera image docs](https://docs.linuxserver.io/images/docker-opera/)
...
- Opera is memory-hungry: give the service 1–2 GB. Heavy tab hoarding needs more.
- Opera ships a built-in VPN, ad blocker and sidebar messengers.
...
## Why Deploy Opera on Railway?

Railway is a singular platform to deploy your infrastructure stack. Railway will host your infrastructure so you don't have to deal with configuration, while allowing you to vertically and horizontally scale it.

By deploying Opera on Railway, you are one step closer to supporting a complete full-stack application with minimal burden. Host your servers, databases, AI agents, and more on Railway.
